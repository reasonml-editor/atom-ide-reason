...
debug('merge', deepmerge_1.default);
...
class ReasonMLLanguageClient extends atom_languageclient_1.AutoLanguageClient {
    ...
    mapConfigurationObject(config) {
        ...
        debug('mapConfigurationObject', config, this.confPerProject);
        return {
            reason: {
                codelens: {
                    enabled: false,
    ...
    startServerProcess(projectPath) {
        ...
        const envPath = process.env.PATH +
            (os.platform() === 'darwin'
                ...
                : '');
        debug('start', projectPath, envPath, confFromPkg, confFromFile, this.confPerProject);
